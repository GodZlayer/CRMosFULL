...
export function createAppRepository(options = {}) {
  ...
  function seedStoreCashAccounts(storeId) {
    const timestamp = nowIso();
    for (const account of STORE_CASH_ACCOUNT_SEEDS) {
      const existing = get(
  ...
  function resolveCashAccountId(storeId, explicitId = null, paymentMethod = "") {
    ...
    let preferredCode = "OUTROS";
    if (method === "DINHEIRO") {
      preferredCode = "DINHEIRO";
    } else if (method === "PIX" || method === "TRANSFERENCIA") {
      preferredCode = "PIX_PJ";
    } else if (method === "CARTAO") {
      preferredCode = "MAQUINA";
    }
  ...
  function saveStoreCashMovement(payload = {}) {
    ...
    const cashAccountId = resolveCashAccountId(
      store.id,
      payload.cashAccountId || payload.cash_account_id,
  ...
  function openCashSession(payload = {}) {
    ...
    const openingAmount = toNumber(payload.openingAmount) ?? 0;
    const timestamp = nowIso();
    const result = run(
      `
    ...
    if (openingAmount > 0) {
      saveStoreCashMovement({
        ...
        entryType: "RECEITA",
        description: `Abertura do caixa da loja ${store.name}`,
  ...
  function closeCashSession(sessionId, payload = {}) {
    ...
    const expectedAmount = Number(movementTotals.total_revenue || 0) - Number(movementTotals.total_expense || 0);
    const closingAmount = toNumber(payload.closingAmount) ?? expectedAmount;
  ...
  function getFinanceWorkbookView(filters = {}) {
    ...
    const ledger = listStoreCashMovements({ ...filters, storeId: store.id });
    const financeEntries = repo.listFinanceEntries({ ...filters, storeId: store.id });
    const accounts = listStoreCashAccounts(store.id);
    const manualSection = (entry) => normalizeText(entry.legacy_section, "ENTRADAS_SAIDAS") || "ENTRADAS_SAIDAS";
    const workbookName = "26 CX Loja ok em 29 02.ods";

    return {
      store,
      accounts,
      ledger,
      entriesAndExpenses: financeEntries.filter((entry) => {
        const section = manualSection(entry);
        return section !== "COMPRAS" && section !== "FLUXO_CAIXA";
      }),
      purchases: financeEntries.filter((entry) => manualSection(entry) === "COMPRAS"),
      importSummary: getLegacyImportSummary({ sourceWorkbook: workbookName })
    };
  }
  ...
  function createPosSale(payload = {}) {
    ...
    if (!clientId) {
      throw new Error("Selecione ou cadastre um cliente para finalizar a venda.");
    }

    const client = get("SELECT id, name, phone, email FROM clients WHERE id = :id", { id: clientId });
    if (!client) {
    ...
    return transaction(() => {
        ...
        run(
          `
            INSERT INTO pos_sale_items (
              sale_id, catalog_item_id, item_name, sku, quantity, unit_cost, unit_price, line_total, created_at
            )
            VALUES (
              :saleId, :catalogItemId, :itemName, :sku, :quantity, :unitCost, :unitPrice, :lineTotal, :createdAt
            )
          `,
          {
            saleId,
      ...
      const payments = Array.isArray(payload.payments) && payload.payments.length
        ? payload.payments
        : [{ paymentMethod: normalizeText(payload.paymentMethod, "DINHEIRO") || "DINHEIRO", amount: totalAmount }];

      for (const payment of payments) {
        const normalizedPaymentMethod = normalizeText(payment.paymentMethod, "DINHEIRO") || "DINHEIRO";
      ...
      saveFinanceEntry({
        ...
        paymentMethod: normalizeText(payments[0]?.paymentMethod, "DINHEIRO") || "DINHEIRO",
        orderId: null,
        storeId: store.id,
  ...
  function listCompanyProfiles(companyCode) {
    ...
    const configuredRows = rows.filter((row) => configuredEmails.includes(String(row.email || "").toLowerCase()));
    configuredRows.sort((left, right) => {
      const leftIndex = configuredEmails.indexOf(String(left.email || "").toLowerCase());
      const rightIndex = configuredEmails.indexOf(String(right.email || "").toLowerCase());
      return leftIndex - rightIndex;
    });
    return configuredRows.map(mapProfile);
  }
  ...
  function saveCatalogItem(payload = {}) {
    ...
    const item = baseSaveCatalogItem(payload);
  ...
  function replenishCatalogItem(itemId, payload = {}) {
    ...
    const item = baseReplenishCatalogItem(itemId, { ...payload, _actor: actor });
  ...
  function saveFinanceEntry(payload = {}) {
    ...
    const entry = baseSaveFinanceEntry({
      ...
      cashAccountId: payload.cashAccountId || payload.cash_account_id || resolveCashAccountId(store.id, null, payload.paymentMethod),
      legacySection
    });
  ...
  function saveTask(payload = {}) {
    ...
    const status = normalizeText(payload.status, existing?.status || "PENDENTE") || "PENDENTE";
    ...
    const normalizedPayload = {
      ...
      valueAmount: toNumber(payload.valueAmount ?? payload.value_amount) ?? existing?.value_amount ?? null,
        ...
        const result = run(
          `
            INSERT INTO daily_tasks (
              ...
              client_name, phone, value_amount, device, contact_channel, notes, source_workbook,
            ...
            VALUES (
              ...
              :clientName, :phone, :valueAmount, :device, :contactChannel, :notes, :sourceWorkbook,
  ...
  function findCatalogMatch({ sku = "", legacyId = "", name = "" } = {}) {
  ...
  function parseLegacyTaskLane(value = "", taskDate = "") {
    const raw = normalizeLegacyText(value);
    ...
    if (/^0\b/.test(raw) || slug.includes("hj")) {
      priorityOrder = 0;
      targetDate = taskDate;
      queueLabel = taskDate ? `Para ${formatShortDate(taskDate)}` : "Para hoje";
      statusLabel = "Hoje";
    } else if (/^1\b/.test(raw)) {
      priorityOrder = 1;
      targetDate = addDaysToDate(taskDate, 1);
      queueLabel = targetDate ? `Para ${formatShortDate(targetDate)}` : "Para 1 dia";
      statusLabel = "1 dia";
    } else if (/^2\b/.test(raw)) {
      priorityOrder = 2;
      targetDate = addDaysToDate(taskDate, 2);
      queueLabel = targetDate ? `Para ${formatShortDate(targetDate)}` : "Para 2 dias";
      statusLabel = "2 dias";
    } else if (/^3\b/.test(raw) || slug.includes("7d")) {
      priorityOrder = priorityOrder ?? 3;
      targetDate = addDaysToDate(taskDate, 7);
      queueLabel = targetDate ? `Para até ${formatShortDate(targetDate)}` : "Para até 7 dias";
      statusLabel = "< 7 dias";
    } else if (/^4\b/.test(raw) || slug.includes("sem")) {
      priorityOrder = priorityOrder ?? 4;
      queueLabel = "Para semana seguinte";
      statusLabel = "Semanal";
  ...
  function importServicesWorkbook(filePath, actor, store) {
    ...
    for (const sheet of workbook.sheets) {
      const isActiveSheet = legacySlug(sheet.name) === "atual";
      const taskDate = parseLegacySheetDate(sheet.name);
      let headerRowIndex = -1;
      let headerMap = {};

      for (let index = 0; index < Math.min(sheet.rows.length, 12); index += 1) {
      ...
      for (let rowIndex = headerRowIndex + 1; rowIndex < sheet.rows.length; rowIndex += 1) {
        ...
        const responsibleName = parseResponsibleName(rawValues[0] || "");
        const legacyLane = parseLegacyTaskLane(rawValues[0] || "", taskDate);
        const name = readCell(row, headerMap, ["nome"]);
        const phone = readCell(row, headerMap, ["tel", "telefone"]);
        const value = coerceLegacyNumber(readCell(row, headerMap, ["vlr", "valor"]));
        const device = readCell(row, headerMap, ["dispositivo"]);
        const services = readCell(row, headerMap, ["servicos", "servico"]);
        const otherNotes = [
          readCell(row, headerMap, ["status"]),
          readCell(row, headerMap, ["descricao de valores"]),
          readCell(row, headerMap, ["outros"]),
          readCell(row, headerMap, ["outros "])
        ].filter(Boolean);
        const updateParts = Object.entries(headerMap)
          .filter(([key]) => key.startsWith("atualizacao"))
          .map(([, index]) => normalizeLegacyText(row[index] || ""))
          .filter(Boolean);
        const orderRef = readCell(row, headerMap, ["os", "ficha"]);
        const orderMatch = findOrderByLegacyReference(orderRef);
        const isInternalTask = legacySlug(name).includes("loja");
        const titleBase = name || services || otherNotes[0] || `Tarefa importada ${sheet.name}`;
        const title = isInternalTask ? `Loja | ${titleBase}` : titleBase;
        const description = [services, ...otherNotes].filter(Boolean).join(" | ");
        const channel = rawValues.some((value) => legacySlug(value).includes("whatsapp") || legacySlug(value) === "zap") ? "WHATSAPP" : "";
        const structuredRow = {
          title,
          description,
          taskDate,
        ...
        const task = saveTask({
          ...
          taskDate,
          status: "PENDENTE",
          priority: isInternalTask ? legacyLane.inferredPriority || "MEDIA" : legacyLane.inferredPriority || "ALTA",
          responsibleName,
          clientName: name,
          phone,
          valueAmount: value,
          ...
          legacyTargetDate: legacyLane.legacyTargetDate,
          rawPayload: { row: rawValues, orderRef },
          initialUpdate: updateParts.join(" | "),
          _actor: actor
        });
    ...
    for (const sheet of workbook.sheets) {
      ...
      if (section === "GERENCIA_CAIXA") {
        ...
        for (let rowIndex = 0; rowIndex < sheet.rows.length; rowIndex += 1) {
          const row = sheet.rows[rowIndex];
          const rawValues = row.map((value) => normalizeLegacyText(value)).filter(Boolean);
          if (!rawValues.length) {
            continue;
          }
          const joined = legacySlug(rawValues.join(" "));
          const account = accounts.find((item) => {
            const nameSlug = legacySlug(item.name);
            return joined.includes(nameSlug) || (item.code === "PIX_PJ" && joined.includes("pix pj")) || (item.code === "MAQUINA" && joined.includes("maq"));
          });
          if (!account) {
            continue;
          }
          const numericValue = [...row].map(coerceLegacyNumber).find((value) => value !== null);
          ...
          run(
            ...
            {
              id: account.id,
              baselineAmount: numericValue,
              sourceWorkbook: workbookName,
              sourceSheet: sheet.name,
              sourceRow: rowIndex + 1,
              snapshotRawPayload: serializeStructuredPayload({ row: rawValues }, "{}"),
              snapshotUpdatedAt: nowIso(),
              updatedAt: nowIso()
            }
          );
          recalculateStoreCashAccountBalance(account.id);
          summary.accountSnapshots += 1;
          recordLegacyImportRow(importRunId, {
            sourceWorkbook: workbookName,
            sourceSheet: sheet.name,
            sourceRow: rowIndex + 1,
            ...
            entityId: account.id,
            structuredPayload: get("SELECT * FROM store_cash_accounts WHERE id = :id", { id: account.id }),
            rawPayload: { row: rawValues }
          });
        }
        continue;
      }
      ...
      if (section === "ESTOQUE") {
        ...
        for (let rowIndex = headerRowIndex + 1; headerRowIndex >= 0 && rowIndex < sheet.rows.length; rowIndex += 1) {
          const row = sheet.rows[rowIndex];
          const rawValues = row.map((value) => normalizeLegacyText(value));
          const legacyId = readCell(row, headerMap, ["id"]);
          ...
          const matched = findCatalogMatch({ legacyId, name });
  ...
  function importLegacyOds(payload = {}) {
    ...
    for (const filePath of requestedFiles) {
      ...
      if (legacySlug(name).includes("servicos")) {
