...
export function createRepository(options = {}) {
  ...
  function listClients(filters = {}) {
    ...
    return rows.filter((row) => matchesSearch(`${row.name} ${row.phone} ${row.email} ${row.document}`, filters.search));
  }
      ...
      run(
        `
          ...
          SET name = :name, phone = :phone, email = :email, document = :document,
              address = :address, notes = :notes, updated_at = :updatedAt
          WHERE id = :id
        `,
        {
          ...normalized,
  ...
  function replenishCatalogItem(itemId, payload = {}) {
    ...
    if (quantity <= 0) {
      throw new Error("Informe a quantidade da reposicao.");
    }
    ...
    const result = run(
      `
        INSERT INTO service_catalog (
          name, description, price_amount, estimated_minutes, active, created_at, updated_at
        )
        VALUES (
          :name, :description, :priceAmount, :estimatedMinutes, :active, :createdAt, :updatedAt
        )
      `,
      {
        ...normalized,
  ...
  function getOrder(orderId) {
    ...
    return {
      ...
      estimated_total_minutes: services.reduce((total, item) => total + Number(item.estimated_minutes || 0), 0),
  ...
  function saveOrder(payload) {
    ...
    const serviceAmount = preparedServices.reduce((total, item) => total + item.unitPrice, 0);
    const totalEstimatedMinutes = preparedServices.reduce((total, item) => total + item.estimatedMinutes, 0);
    const quoteAmount = partsSubtotal + serviceAmount;
    ...
    return transaction(() => {
        ...
        run(
          `
            INSERT INTO order_services (
              order_id, service_id, service_name, description, estimated_minutes, quantity, unit_price
            )
            VALUES (
              :orderId, :serviceId, :serviceName, :description, :estimatedMinutes, :quantity, :unitPrice
            )
          `,
          {
            orderId,
      ...
      for (const requestedProduct of requestedProducts) {
        run(
          `
            INSERT INTO order_requested_products (order_id, product_name, created_at, updated_at)
            VALUES (:orderId, :productName, :createdAt, :updatedAt)
          `,
          {
            orderId,
